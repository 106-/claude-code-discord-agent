...
from claude_code_sdk import query, ClaudeCodeOptions, AssistantMessage, TextBlock, ToolUseBlock, ToolResultBlock, UserMessage, ResultMessage
    ...
    async def handle_mention(self, message):
        ...
        try:
            ...
            async with message.channel.typing():
                ...
                async for response in query(prompt=user_content, options=self.claude_options):
                    self.logger.info(f"Received response type: {type(response)}")
                    self.logger.info(f"Received response: {json.dumps(asdict(response), indent=2)}")
